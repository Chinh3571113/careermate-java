...
@WebMvcTest(CandidatePaymentController.class)
@AutoConfigureMockMvc(addFilters = false)
@DisplayName("CandidatePaymentController Tests")
class CandidatePaymentControllerTest {
    ...
    @MockBean
    private CandidatePaymentImp candidatePaymentImp;
    ...
    @Nested
    @DisplayName("POST /api/candidate-payment - Create Payment")
    class CreatePaymentTests {
        @Test
        @DisplayName("Should create payment URL successfully")
        void createPayment_ReturnsSuccess() throws Exception {
            when(candidatePaymentImp.createPaymentUrl(any(HttpServletRequest.class), anyString()))
                    .thenReturn("https://payment.example.com/pay/123");

            mockMvc.perform(post("/api/candidate-payment")
